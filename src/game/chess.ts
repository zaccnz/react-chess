...
export type PlayerType = 'local' | 'bot' | 'online-local' | 'online-network';
...
const IndexToXY = (state: ChessState, index: number): [x: number, y: number] => {
  return [index % BOARD_COLUMNS, Math.floor(index / BOARD_COLUMNS)];
};
...
export const ChessInitializeState = (config: ChessConfig, username = '', length = 10): ChessState => {
  const state: ChessState = {
    board: [...Array(BOARD_ROWS * BOARD_COLUMNS)].map(() => null),
    ...
    players: [
      {
        ...
        name: config.player_white === 'online-local' ? username : 'WHITE',
        lost_pieces: [],
        check: false,
        checkmate: false,
        time_left: length,
      },
      {
        ...
        name: config.player_black === 'online-local' ? username : 'BLACK',
        lost_pieces: [],
        check: false,
        checkmate: false,
        time_left: length,
...
export const ChessCanUndo = (state: ChessState): boolean => {
  for (const player of state.players)
    if (player.type === 'online-local' || player.type === 'online-network')
      return false;
...
export const ChessDoMove = (state: ChessState, move: ChessMove): void => {
  ...
  if (move.takes && takes) {
    state.players[takes.team].lost_pieces.push(takes.type);
  }
...
export const ChessUndoMove = (state: ChessState, move: ChessMove): void => {
  ...
  if (move.takes) {
    ...
    state.players[team].lost_pieces.splice(state.players[team].lost_pieces.indexOf(move.takes.piece), 1);
  }
...
export const ChessCheckWin = (state: ChessState): void => {
  const checks = ChessIsCheck(state);
  const checkmates = ChessIsCheckmate(state);
  for (let i = 0; i < Team.MAX; i++) {
    state.players[i].check = checks[i];
    state.players[i].checkmate = checkmates[i];
  }
};
...
export const ChessGetPossibleMoves = (state: ChessState, from_x: number, from_y: number, not_check = true): PotentialMove[] => {
  ...
  const inBounds = (to_x: number, to_y: number) =>
    !(to_x < 0 || to_x >= BOARD_COLUMNS || to_y < 0 || to_y >= BOARD_COLUMNS);
  ...
  switch (piece.type) {
    ...
    case 'king': {
      ...
      if (rook && rook.type == PieceType.Rook && !rook.has_moved) {
        let can_castle = !ChessIsCheck(state)[piece.team];
        for (let i = 5; i < 7; i++) {
          can_castle = can_castle && state.board[XYtoIndex(state, i, from_y)] == null;
        }
        for (let i = 5; i < 7; i++) {
          can_castle = can_castle && !ChessMoveIsCheck(state, from_x, from_y, i, from_y)[piece.team];
        }
        if (can_castle) {
